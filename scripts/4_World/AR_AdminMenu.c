class AR_AdminMenu extends UIScriptedMenu
{
    ...
    override Widget Init ()
    {
        ...
        {
            layoutRoot                      =   GetGame().GetWorkspace().CreateWidgets("AR_Banco/GUI/layouts/BancoAvanzadoMenu.layout");
    ...
    override bool OnClick(Widget w, int x, int y, int button)
    {
        switch(w)
        {
            ...
            case m_BtnSavePlayerData:
                ...
                break;
            case m_ReloadButton:
                GetRPCManager().SendRPC("AR_Banco", "AdminReloadConfig", null, true);
                break;
    ...
    {
        AR_BancoConfigManager cfg = Nuevo AR_BancogConfigManager;
    ...
    void HandleShowATMOnMap()
    {
		...
		m_MapWidget.AddUserMark(LocalSpawnMarkerPos, "Selected ATM is here", ARGB(255,255,0,0), "\\AR_Banco\\GUI\\textures\\ATMMapicon.paa");
    ...
    void LoadAllATMs()
    {
        ...
        for(int i = 0; i < class_name_count; i++)
        {
            ...
            if(GetGame().IsKindOf(classname,"AR_Banco"))//Check if Base class is Banking Base.
            {
